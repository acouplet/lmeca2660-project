...
void Problem_Poisson(Problem *This){
    ...
    while(This->globe > 1e-6){
        FR(i,1,Ny+1){
            FR(j,1,Nx+1){
                Phistar[i][j] = 0.25*h*h*(-1/This->dt*((ustar[i][j]-ustar[i][j-1])/h + (vstar[i-1][j]-vstar[i][j])/h) + (Phi[i+1][j] + Phi[i-1][j])/(h*h) + (Phi[i][j+1] + Phi[i][j-1])/(h*h));
                Phi[i][j] = This->alpha*Phistar[i][j] + (1.0-This->alpha)*Phi[i][j];
            }
        }
